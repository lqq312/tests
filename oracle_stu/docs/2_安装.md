...
如使用图形化安装则需要在图形化界面执行“runInstaller”的命令。
...
如使用图形化工具安装监听器则需要以oracle用户的身份执行“netca”即可。
...
如使用图形化工具安装数据库则直接以oracle用户的身份执行“dbca”命令即可。
